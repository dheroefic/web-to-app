...
社区市场只承载 JS/CSS 扩展模块。浏览器扩展（MV3 Chrome 扩展）不再是社区目录——**浏览器扩展** Tab 直接实时搜索 Chrome 网上应用店：输入关键词、浏览结果、通过现有的 CRX 下载链路按需安装。如果实时搜索不可达，也可以粘贴商店链接或 32 位扩展 ID 直接安装。实时搜索需要能访问 Google 的网络。
